...
import { ofertaApi, trabajadorApi } from '@/lib/api'
  ...
  const loadData = async () => {
    try {
      if (isSupabaseConfigured && supabase) {
        ...
        if (user) {
          const [ofertasData, trabajadorData] = await Promise.all([
            ofertaApi.list({ estado: 'ABIERTA' }),
            trabajadorApi.get(user.id).catch(() => null),
          ])
          setOfertas(ofertasData?.slice(0, 5) || [])
          setTrabajador(trabajadorData)

          setStats({
            disponibles: ofertasData?.length || 0,
            postuladas: 0,
            aceptadas: 0,
          })
        }
